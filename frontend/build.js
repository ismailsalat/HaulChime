...
const SITE = {
  ...
  email: process.env.PUBLIC_EMAIL || local.email || 'asalat@haulchime.com',
  // The partner portal is served by the Flask app, not by the static site, so
  // it lives on the API host unless a dedicated domain is configured.
  partnerUrl: process.env.PARTNER_URL || local.partnerUrl ||
              (process.env.API_URL || local.apiUrl || 'http://localhost:5002'),
